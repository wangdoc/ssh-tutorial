...
第一步，手动将客户端的公钥放入远程服务器的指定位置。(建议参照下文的 `ssh-copy-id` 上传公钥)
...
### ssh-add 配合 ssh-agent 的用法举例

参考: https://www.ssh.com/academy/ssh/agent

```bash
eval `ssh-agent` && ssh-add # 启动ssh-agent 并且把当前用户的key加到agent里
...
假设 本机   可以ssh到 host_b  (即 host_b上有本机的公钥)
假设 host_a 不可以ssh到 host_b(即 host_b上没有host_a的公钥)

使用上面的方法, 可以实现在 host_a 上, 直接ssh登录 host_b(host_b上没有host_a的公钥)
